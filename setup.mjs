...
export async function setup(ctx) {
  ...
  const conquestGamemodeCheck = (gamemode = game.currentGamemode) => { // Check if the user is playing a CO game mode currentGamemode.id.includes("conquest")
    return gamemode.id.includes("conquest")
  }

  //Lifecycle Hooks
  ctx.onCharacterLoaded((ctx) => {
    if (conquestGamemodeCheck()) {      //Keeps the mod from breaching containment
  ...
  ctx.patch(Player, "autoEat").replace(function (o, foodSwapped) {
    ...
    if ((this.hitpoints <= this.autoEatThreshold || foodSwapped) &&
